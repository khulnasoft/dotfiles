...
if [ "$1" = "personal" ]; then
	...
	nvim -p .vimrc .config/nvim/init.vim .config/nvim/coc-settings.json .zshrc .gitconfig .gitignore .zprofile .tmux.conf .tmux.conf.settings tools/bin/executable_autoupdate.zsh tools/bin/executable_sync_brews.sh
fi
